                ...
                div { class: "bg-gray-800 border border-gray-700 rounded-xl p-3 flex flex-col items-center shadow-lg transition-transform hover:scale-105",
                    {
                        ...
                        if let Some(url) = image_url {
                            rsx! { img { src: "{url}", alt: "{entry.card.name}", class: "w-full rounded-lg mb-3 shadow-md border border-gray-600 aspect-[63/88] object-cover", loading: "lazy" } }
                        } else {
                            rsx! { div { class: "w-full aspect-[63/88] bg-gray-700 rounded-lg mb-3 flex items-center justify-center border border-gray-600 animate-pulse", span { class: "text-4xl opacity-50", "🃏" } } }
                        }
                    }
                        ...
                        for (owner_name, count) in entry.owners.iter() {
                            if *count > 0 {
                                // Escaping into pure Rust to clone variables safely!
                                {
                                    ...
                                    rsx! {
                                        div { class: "group relative flex items-center gap-1 bg-blue-900/50 text-blue-200 border border-blue-700/50 pl-2 pr-1 py-0.5 rounded-full font-mono text-[10px] transition-all hover:bg-blue-800",
                                            span { "{owner_name}: {count}" }
                                            
                                            // Hidden 'X' button that appears on hover
                                            button {
